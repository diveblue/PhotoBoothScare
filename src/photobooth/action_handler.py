...
class ActionHandler:
    ...
    def _execute_action(self, action, frame, width, height, now):
        ...
        gotcha_active = hasattr(action, "show_gotcha") and action.show_gotcha
        ...
        if render_state["gotcha_active"]:
            print("👻 Showing gotcha overlay")
        if render_state["qr_url"]:
            print("🔗 Showing QR overlay")

        print(
            f"🎨 Calling overlay with phase={render_state['phase']}, countdown={render_state['countdown_active']}, gotcha={render_state['gotcha_active']}"
        )
